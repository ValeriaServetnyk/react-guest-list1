...
return (
  <div className='Main'>
    <div className='textContainer'>
      ...
      <p>After successfully finishing the Upleveled bootcamp I got my first amazing job offer and since now I have money I can throw a massive party.</p>
...
if( event.key === 'Enter')
{event.preventDefault();
createGuest ([firstName, lastName]).catch(() => {console.log('guest not added');
});
}
}}
/>
      ...
      <div key={guest.id} data-test-id="guest">
        ...
        <div className='generatedList'>
          ...
          <label>
            ...
            onChange={() => {updateAttendance(guest.id, !guest.attending).catch(() => { console.log('guest not updated');
          })
...
onClick={() => {deleteGuest(guest.id).catch(() => {console.log('guest not deleted');
})
}}>
  {' '}
  Remove
